...
function ChampBravoMapping_A321_toliss()
  ...
  do_every_frame([[
      ParkPos = get("AirbusFBW/ParkBrake")
      SwPos = button(]]..btq.sw1_up..[[)
      if (SwPos and (ParkPos < 1)) then
        command_once("toliss_airbus/park_brake_set")
      elseif (not(SwPos) and ParkPos) then
...
function ChampBravoCheck_A321_toliss()
  if (--AP Panel
      ...
      --Switches
      XPLMFindDataRef("AirbusFBW/ParkBrake")                                    ~= nil and
      XPLMFindCommand("toliss_airbus/park_brake_set")                           ~= nil and
      XPLMFindCommand("toliss_airbus/park_brake_release")                       ~= nil
